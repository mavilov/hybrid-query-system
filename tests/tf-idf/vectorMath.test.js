...
import { magnitude, dotProduct, cosineSimilarity } from '../../src/tf-idf/vectorMath.js'
...
suite('dotProduct()', () => {
    ...
    test('warns and throws an exception for mismatched lengths', () => {
        ...
        try {
            const vecA = [1, 2, 3]
            const vecB = [1, 2]
            assert.throws(() => dotProduct(vecA, vecB), /Vector length mismatch/)
...
suite('cosineSimilarity()', () => {
    ...
    test('is symmetric', () => {
        const vecA = [1, 2, 3]
        const vecB = [4, 5, 6]
        const result1 = cosineSimilarity(vecA, vecB)
        const result2 = cosineSimilarity(vecB, vecA)
    ...
    test('real-world TF-IDF example', () => {
        ...
        const result = cosineSimilarity(doc1Vector, doc2Vector)
